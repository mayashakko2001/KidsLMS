...
class ArcheivementController extends Controller
{
   ...
   public function game_result(){
    ...
    foreach( $game_result as $ga){
     ...
     $archeivements = DB::table('archeivements')->where('student_id', $ga->student_id)
     ->where('level_id', $ga->level_id)->first();
    $total=$ga->first_game+$ga->second_game+$ga->third_game;
    $result[]=[
        //'id'=>$student->id,
        'level_id'=>$ga->level_id,
        'id'=>$ga->id,
        ...
        'second_game'=>$ga->second_game,
        'third_game'=>$ga->third_game,
        'total'=> $archeivements->total_scores
      //  'fourth_game'=>$ga->fourth_game,
   ...
   public function update_points(Request $request,$id){
    $ga =Game_result::find($id);
    $archeivements = DB::table('archeivements')->where('student_id', $ga->student_id)
    ->where('level_id', $ga->level_id)->first();
    $ar =Archeivement::find($archeivements->id);
    $ar->total_scores = $request->total;
    $ar->save();
    return $this->success( '' ,200,'points add successfully');
   }
}
