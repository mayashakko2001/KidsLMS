...
public function update(Request $request, $id)
{
 ...
 public function store_student_answers(Request $request){
    $student=auth()->user();
    $request->validate([
        'quiz_id' => 'required',
    ...
    $an= $input['answer'] = array($input['answer']);
    $test1=array_shift($an);

    foreach($test1 as $an_st){
     $stu_an= new Student_answer();
     $stu_an->quiz_id= $request->quiz_id;
     $stu_an->student_id=  $student->id;
     $stu_an->question_number=$an_st['question_number'];
     $stu_an->answer=$an_st['answer'];
        $stu_an->save();

    }

    return $this ->success('',200,'add successfully');

 }
   public function get_student_answers(){
   $stu= Student_answer::all();
   if(empty( $stu)){
   ...
   foreach($stu as $st){
    ...
    $result[]=[
      ...
      'quiz_id '=>$st->quiz_id,
      'question_number'=> $st->question_number,
       'answer'=>$st->answer
    ];
